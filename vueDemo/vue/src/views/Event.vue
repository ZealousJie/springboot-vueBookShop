<template>
  <div style="padding: 10px">
    ...
    <div style="margin: 10px 0">
        <el-select v-model="form.eventType" placeholder="赛事类型" style="margin: 10px 10px 10px 0" @change="load"
      ...
      <el-select v-model="form.eventProject" placeholder="赛事项目"
        filterable clearable style="margin: 10px 10px 10px 0" @change="load">
        <el-option
      ...
      <el-select v-model="form.organizer" placeholder="举办方"
                 filterable clearable style="margin: 10px 10px 10px 0" @change="load">
        <el-option
            v-for="(item,i) in organizerOptions"
            :key="i"
    ...
    <el-table :data="tableData" style="width: 99%;" stripe border size="18px" @row-dblclick="eventNews"
      ...
      <el-dialog v-model="vis" title="赛事详情" width="50%">
        <el-form :model="detail" label-width="120px">
          ...
          <el-form-item label="参赛团队" size="18px">
            <el-dropdown>
              <el-button type="success" size="medium">
                参赛列表<i class="el-icon-arrow-down el-icon--right"></i>
              </el-button>
              <template #dropdown>
                <el-dropdown-menu slot="dropdown">
